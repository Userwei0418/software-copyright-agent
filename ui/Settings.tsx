...
import senseaudioCatalog from "../shared/senseaudio-models.json";
...
const providerPresets: ProviderPreset[] = [
  { id: "senseaudio", label: "商汤 SenseAudio", name: "商汤 SenseAudio", protocol: "openai_compatible",
    baseUrl: senseaudioCatalog.baseUrl, models: senseaudioCatalog.models,
    hint: `官方模型目录更新于 ${senseaudioCatalog.updatedAt}；图片能力通过实际测试后启用`, maxConcurrency: 3 },
...
export function Settings({ connection }: { connection: SidecarConnection | null }) {
  ...
  const modelOperations = useRef(new Set<string>());
  const [busyModels, setBusyModels] = useState<string[]>([]);
  ...
  async function refresh(resetPreferences = false) {
  ...
  async function initialize() {
    ...
    try { await refresh(true); }
      ...
      if (request !== settingsRequest.current) return;
      const detail = `设置读取失败：${errorText(error, "无法连接本地数据库")}`;
      setMessage(detail); setLoadError(detail); setLoaded(true);
    }
  }
  ...
  async function tryDiscover() {
    if (operationPending.current || modelOperations.current.size) return;
    ...
    // Discovery must not replace a saved connection credential before Save.
    const temporaryCredentialId = protocol !== "ollama" && apiKey.trim() ? crypto.randomUUID() : null;
    const probeId = temporaryCredentialId || providerId.current;
    try {
      if (temporaryCredentialId) await storeModelCredential(temporaryCredentialId, apiKey);
      const result = await probeModelConfig({ configId: probeId, protocolId: protocol,
        baseUrl, modelName: "" });
      ...
      if (temporaryCredentialId) {
        try { await deleteModelCredential(temporaryCredentialId); }
        catch (error) { setMessage(errorText(error, "临时探测凭据清理失败")); }
      }
      operationPending.current = false; setBusy(null);
    }
  }

  async function saveProvider() {
    if (operationPending.current || modelOperations.current.size || !loaded || loadError) return;
    ...
    finally { operationPending.current = false; setBusy(null); }
  }

  async function savePreferences() {
    if (!connection || !loaded || loadError || operationPending.current) return;
    ...
    operationPending.current = true; setBusy("preferences");
  ...
  async function editProvider(group: ModelConfig[]) {
    ...
    let hasKey = !first.has_credential;
    try { hasKey = hasKey || await hasModelCredential(first.provider_id); }
    catch (error) { setMessage(errorText(error, "无法读取此连接的凭据状态")); return; }
  ...
  async function removeProvider(group: ModelConfig[]) {
    if (!connection || operationPending.current || modelOperations.current.size || !window.confirm(`删除连接“${group[0].name}”及其 ${group.length} 个模型？`)) return;
    operationPending.current = true; setBusy("remove");
    try {
  ...
  function beginModelOperation(id: string) {
    if (!connection || operationPending.current || modelOperations.current.has(id)) return false;
    modelOperations.current.add(id); setBusyModels([...modelOperations.current]);
    return true;
  }

  function finishModelOperation(id: string) {
    modelOperations.current.delete(id); setBusyModels([...modelOperations.current]);
  }

  async function testSavedModel(item: ModelConfig) {
    if (!beginModelOperation(item.id)) return;
    ...
    } finally { finishModelOperation(item.id); }
  }

  async function toggleVisionCapability(item: ModelConfig) {
    if (!connection || !beginModelOperation(item.id)) return;
  ...
  const providerBusy = !!busy || busyModels.length > 0;
  ...
  const providerForm = <section className="provider-editor"><div className="editor-title">
    <button className="back-link" disabled={providerBusy} onClick={() => { if (editingProviderId) setEditingProviderId(null); else setAddStep("catalog"); }}>← 返回</button>
    ...
    <div className="connection-form-grid"><label>连接名称<input disabled={!!busy} required value={name} onChange={(event) => setName(event.target.value)} /></label>
      {(presetId === "custom" || editingProviderId) && <><label>接口协议<select disabled={!!busy} value={protocol} onChange={(event) => { const value = event.target.value as typeof protocol; setProtocol(value); setBaseUrl(defaults[value]); }}>
        ...
        <label className="wide-field">请求地址（Base URL）<input disabled={!!busy} required value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label></>}
      {presetId !== "custom" && !editingProviderId && <div className="preset-summary wide-field"><small>请求地址</small><strong>{baseUrl}</strong></div>}
      {protocol !== "ollama" && <label className="wide-field">API Key<input disabled={!!busy} type="password" autoComplete="new-password" value={apiKey} onChange={(event) => setApiKey(event.target.value)}
      ...
      <label className="wide-field">模型 ID <small>每行一个；可随时增删</small><textarea disabled={!!busy} rows={7} value={modelText} onChange={(event) => setModelText(event.target.value)} /></label></div>
    <div className="editor-actions"><button className="secondary-action" disabled={providerBusy || !connection || !loaded || !!loadError} onClick={tryDiscover}>{busy === "detect" ? "正在获取…" : "自动获取模型"}</button>
      <button disabled={providerBusy || !connection || !loaded || !!loadError} onClick={saveProvider}>{busy === "save" ? "正在保存…" : `${editingProviderId ? "保存修改" : "添加服务"}（${enteredModels().length} 个模型）`}</button></div>
  </section>;
  return <main className="settings-page"><header className="topbar"><div><p className="eyebrow">SETTINGS</p>
    ...
    <section className="settings-workbench"><aside className="provider-nav"><div className="provider-nav-title"><div><strong>模型服务</strong><small>{providers.length} 个连接</small></div>
      <button disabled={providerBusy || !loaded || !!loadError} onClick={beginAddProvider}>＋ 添加</button></div>
      <div className="provider-nav-list">{!loaded ? <p>正在读取本地配置…</p> : providers.map((group) => <button className={selectedView === group[0].provider_id ? "active" : ""} key={group[0].provider_id} disabled={providerBusy}
      ...
      <button className={`preference-nav ${selectedView === "preferences" ? "active" : ""}`} disabled={!!busy} onClick={() => setSelectedView("preferences")}><span>⚙</span><div><strong>生成偏好</strong><small>默认模型与参数</small></div></button>
    </aside><div className="settings-detail">{message && <p className="settings-message" role="status" aria-live="polite">{message}</p>}{loadError && <button disabled={!connection} onClick={initialize}>重新读取设置</button>}
      {selectedView === "add" && addStep === "catalog" && <section className="provider-catalog"><div><h2>添加模型服务</h2><p>选择服务商后，只需填写 API Key；模型与地址已为你预设。</p></div>
        <div className="provider-catalog-grid">{providerPresets.map((preset) => <button key={preset.id} disabled={providerBusy || !loaded || !!loadError} onClick={() => choosePreset(preset.id)}><span>{preset.id === "custom" ? "+" : preset.label.slice(0, 1)}</span>
      ...
      {selectedProvider && editingProviderId !== selectedProvider[0].provider_id && <section className="provider-detail"><header><div className="provider-identity"><span>{selectedProvider[0].name.slice(0, 1)}</span><div><h2>{selectedProvider[0].name}</h2><p>{selectedProvider[0].base_url}</p></div></div>
        <div className="provider-actions"><button disabled={providerBusy} onClick={() => editProvider(selectedProvider)}>编辑配置</button><button className="danger" disabled={providerBusy} onClick={() => removeProvider(selectedProvider)}>删除</button></div></header>
        ...
        <div className="provider-models">{selectedProvider.map((item) => <div className="provider-model" key={item.id}><div><strong>{item.model_name}</strong>
          ...
          <div className="model-actions"><button disabled={!!busy || busyModels.includes(item.id) || !connection} onClick={() => testSavedModel(item)}>测试连接</button>
            <button className={item.supports_vision === true && item.vision_verified ? "vision-active" : ""}
              disabled={!!busy || busyModels.includes(item.id) || !connection}
      ...
      <div className="preference-grid"><label>说明书默认模型<select disabled={!!busy} value={preferences.manual_model_id ?? ""}
      ...
      <label>图表语义默认模型<select disabled={!!busy} value={preferences.diagram_model_id ?? ""}
      ...
      <label>生成温度<input disabled={!!busy} type="number" min="0" max="2" step="0.1" value={preferences.temperature}
        onChange={(event) => setPreferences({ ...preferences, temperature: Number(event.target.value) })} /></label>
      <label>最大输出 Token<input disabled={!!busy} type="number" min="1024" max="32768" step="1024" value={preferences.max_output_tokens}
      ...
      <label>默认源码筛选<select disabled={!!busy} value={preferences.source_strategy}
      ...
      <label className="check-setting"><input disabled={!!busy} type="checkbox" checked={preferences.auto_preview}
      ...
      <details className="advanced-prompts"><summary><span><b>高级风格提示词</b><small>专业人员使用 · 修改可能影响生成效果</small></span><em>展开配置</em></summary>
        ...
        <label>文档风格提示词 <small>控制正文语气、段落组织、详略和专业表达</small><textarea disabled={!!busy} rows={7}
        ...
        <label>绘画风格提示词 <small>控制图表类型选择、布局、阅读方向、连线与视觉层级</small><textarea disabled={!!busy} rows={9}
        ...
        <button type="button" disabled={!!busy} onClick={() => setPreferences({ ...preferences,
      ...
      <button disabled={providerBusy || !connection || !loaded || !!loadError} onClick={savePreferences}>{busy === "preferences" ? "正在保存…" : "保存通用设置"}</button>

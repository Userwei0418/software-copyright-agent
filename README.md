...
面向最终用户的主入口是“快速开始”：确认项目、软件名称、版本、截图文件夹和正文/图表/视觉模型后，后台并行生成源代码文档与软件说明书，最后统一装配、质检并交付到“我的资产”。并发数和重试策略收在高级设置中；专业工作台用于查看、修订和单节点恢复。

## 2026-09-11 体验与质量优化

- 导航聚焦“快速开始”和“我的资产”；专业工作台折叠收纳。已有任务先显示当前进度与下一步，启动新任务保留旧记录和资产。
- 网络/进程中断提供继续入口，质量或证据问题优先定位处理。保存、重试和模型测试防止重复提交；网络响应丢失时不隐式重发写请求。
- 预览统一为可用键盘操作的弹窗，关闭后不因迟到响应重新弹出；切项目不串结果。文字最小 12px，重要操作有可见焦点，窄窗口保留主要操作。
- 源码长路径标题纳入分页预算，源码导出增加完整性检查和写后回读；说明书检查覆盖表格、截图事实、目录页码和当前生成规则，旧标准产物需重新装配。
- SenseAudio 的官方 15 个模型保存在 [共享目录](shared/senseaudio-models.json)，设置页与桌面探测回退共用。目录于 2026-09-11 核对；官方列出不代表每个账户均可调用，仍需逐模型测试。凭据通过本地加密存储，编辑连接时的模型发现使用临时凭据，保存前不替换旧 Key。

完整问题、实测证据与平台限制见 [本轮审计](AUDIT_2026-09-11.md)。当前仓库优化与已发布的 v0.1.1 安装包应分别看待；发布与安装验证见 [发布清单](RELEASE_CHECKLIST.md)。

## 当前可运行能力

- “快速开始”无人值守编排：保留历史运行、从失败节点恢复，同一次运行固定绑定一个项目任务和一个说明书任务，不会在重试时无故创建 v2/v3 整单任务。
...
- 运行日志：按最近 3/5/10/20 次快速任务查看阶段状态、累计尝试、阻塞原因与质检失败项；可导出经过路径、凭证和大段项目内容脱敏的 JSON 诊断包。
...
- “装配双文档”属于确定性质量闸门，不会对同一份未变化文档反复自动重试；阻断时保留具体 QA 失败项，显式恢复后优先复用已有候选稿或终稿并按当前策略复检。

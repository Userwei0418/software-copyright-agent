...
class QuickStartUiContractTests(unittest.TestCase):
    ...
    def test_quick_start_requires_explicit_screenshot_authorization(self):
        ...
        self.assertIn("从失败处继续", page)
        self.assertIn("查看与导出文档", page)
        self.assertIn("新建生成任务", page)
        self.assertNotIn("discardQuickStartRun", page)
        self.assertIn("保留本次记录与全部产物", page)
        self.assertIn("quick-advanced-settings", page)
        self.assertIn('aria-label="一行阶段概览"', page)
        self.assertNotIn('<details className="quick-run-details"', page)
    ...
    def test_quick_start_shows_parallel_deliverables_and_real_dependencies(self):
        ...
        self.assertIn("quick-stage-overview", page)
        ...
        dialog = (ROOT / "ui" / "Dialog.tsx").read_text(encoding="utf-8")
